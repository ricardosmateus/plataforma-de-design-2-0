...
> **Versão:** 1.5.0 · **Status:** Implementado · "Editar" fora da interface no lançamento (PROJ-CRIA-009)
...
| PROJ-CRIA-002 | `nome` e `descricao` exibidos no card não são digitados: vêm de um catálogo fixo `tipo → {nome, descricao}` mantido no servidor. ~~O cliente nunca envia nem `nome` nem `descricao`.~~ **Metade revista em 22/09/2026 por PROJ-CRIA-010:** o cliente passa a enviar `nome`, escolhido numa lista fechada na modal. `descricao` continua só do servidor, e o `nome` do catálogo continua valendo para todo projeto que não escolheu um. **Outra metade revista em 23/09/2026 por PROJ-CRIA-011:** no card da listagem, a descrição passa a ser a frase da modal para o nome escolhido; a `descricao` do servidor fica como reserva. | Decorre de P1 · revista 22 e 23/09/2026 |
| PROJ-CRIA-003 | Não há **bloqueio** de duplicidade: a mesma empresa pode ter mais de um projeto do mesmo `tipo` — e, desde PROJ-CRIA-010, do mesmo nome. **Complementada em 23/09/2026 por PROJ-CRIA-012:** repetir o nome continua permitido, mas passa a pedir confirmação antes de criar. | Consequência aceita de P1 · complementada 23/09/2026 |
...
| `nome` | Não | O projeto escolhido na modal (PROJ-CRIA-010). Até 80 caracteres. Ausente: vale o rótulo do tipo. |
| `confirmarDuplicado` | Não | `true` só depois que a pessoa escolheu "Criar mesmo assim" na modal "Projeto já existe" (PROJ-CRIA-012). |

| Situação | Resposta |
|---|---|
| Criado | `201` com o projeto, no formato do `GET /empresas/:empresaId/projetos` |
| A empresa já tem projeto ativo com o mesmo nome e `confirmarDuplicado` não veio `true` (PROJ-CRIA-012) | `409`, `codigo: "projeto_duplicado"`, com `duplicado: { nome, empresa, quantidade, criado_em }` — `criado_em` é o do mais recente. Nada é gravado. |
...
| PROJ-CRIA-010 | A modal "Novo projeto" lista **possíveis projetos em 12 categorias** (Marca, Digital, Produto, Marketing, Conteúdo, IA & Automação, Negócios, Experiência, Comunicação, Operações, Audiovisual, Pesquisa & Inovação). **A lateral lista só as categorias; os projetos da categoria aberta aparecem no conteúdo, como cards de escolha única** — o mesmo card que o "Startup" usava. Cada card traz, abaixo do nome, uma frase curta explicando o que é aquele tipo de projeto ("POC: prova de conceito…") — só na tela; o que vai para a API continua sendo apenas o nome. O "Salvar" fica travado até haver escolha. O que a pessoa escolhe vira o **`nome`** do projeto — nunca o `tipo`. O `tipo` continua `startup`, único habilitado, e continua sendo ele quem decide o que o projeto abre por dentro. Sem escolha, o `nome` vem do catálogo, como antes. | Decidido 22/09/2026 com o Ricardo |
| PROJ-CRIA-011 | **O card de um projeto na listagem mostra a descrição e a ilustração do projeto escolhido, não as do tipo.** Como todo projeto é `tipo: startup` por trás (PROJ-CRIA-010), a `descricao` e a miniatura do tipo saíam iguais para "Logotipo", "Site" e "App". Agora: (a) a descrição é a mesma frase que a pessoa leu no card da modal ao escolher, encontrada pelo `nome` (sem diferenciar acento ou maiúsculas); sem correspondência — projeto antigo, sem nome, ou nome que saiu da lista — vale a `descricao` do servidor; (b) a ilustração vem de um mapa por nome (`THUMBS_POR_NOME` em `projetos.html`); nome sem arte própria cai na miniatura do tipo (PROJ-CRIA-008). As ilustrações entram uma a uma, no traço de `card-startup.svg` (contorno preto, preenchimento branco, azul/amarelo/vermelho). Com arte própria até agora: **Logotipo** (`img/card-logotipo.svg`), **Landing Page** (`img/card-landing-page.svg`) e **Rebranding** (`img/card-rebranding.svg`). | Decidido 23/09/2026 com o Ricardo |
| PROJ-CRIA-012 | **Projeto com nome repetido na empresa pede confirmação.** Ao clicar em "Salvar" na modal "Novo projeto", se a empresa já tem um projeto **ativo** com o mesmo nome, nada é criado e abre a modal **"Projeto já existe"**: "A empresa **X** já tem um projeto **Logotipo**, criado em dd/mm/aaaa. Deseja criar outro mesmo assim?" (com dois ou mais: "já tem N projetos **Logotipo** — o mais recente criado em dd/mm/aaaa"). Duas saídas: **"Criar mesmo assim"** cria o projeto normalmente; **"Cancelar"** — e também "Fechar", Esc e clique fora — cancela a criação inteira: nada é gravado, a modal "Novo projeto" não reabre e a escolha é descartada. O foco inicial fica no "Cancelar". O que conta como "mesmo nome": o nome exibido no card (o escolhido, ou o rótulo do tipo nos projetos antigos sem nome), sem diferenciar acento, maiúsculas e espaços extras. Projetos excluídos não contam. A conferência é do **servidor**, não da lista que a tela tem, porque outro membro pode ter criado o mesmo projeto depois que a página abriu. | Decidido 23/09/2026 com o Ricardo |
...
| P4 | Nome duplicado bloqueia? | Não — não existe nome próprio, só `tipo`, e repetir tipo é permitido (PROJ-CRIA-003). **Revista em 23/09/2026 (P9):** continua sem bloquear, mas passou a haver nome próprio, e repetir pede confirmação. | 24/08/2026 · revista 23/09/2026 |
| P9 | Criar um projeto com um nome que a empresa já tem: bloquear, avisar ou deixar passar? | **Avisar e deixar a pessoa decidir** (PROJ-CRIA-012). Bloquear impediria casos legítimos — uma segunda rodada de Logotipo, uma submarca, uma Landing Page por campanha. Deixar passar calado gera o repetido por engano de quem volta dias depois sem lembrar. O aviso é do servidor, com `409` + confirmação explícita, para valer também quando a lista da tela está desatualizada. | 23/09/2026 |
...
| 1.5.0 | 2026-09-23 | PROJ-CRIA-012: nome de projeto repetido na empresa abre a modal "Projeto já existe", com "Criar mesmo assim" e "Cancelar". `POST` ganha `confirmarDuplicado` e a resposta `409 projeto_duplicado`. PROJ-CRIA-003 e P4 complementadas; decisão P9. §2 passa a listar também o campo `nome`. |
| 1.4.0 | 2026-09-23 | PROJ-CRIA-011: o card da listagem passa a mostrar a descrição da modal para o nome escolhido (antes, a do tipo — igual para todos) e uma ilustração por nome de projeto, começando por Logotipo. PROJ-CRIA-002 revista na outra metade. |
| 1.3.2 | 2026-09-23 | PROJ-CRIA-010: cada card de projeto ganha uma descrição curta abaixo do nome, para quem não conhece o termo saber o que está escolhendo. Só texto de tela — o `nome` enviado não muda. |
