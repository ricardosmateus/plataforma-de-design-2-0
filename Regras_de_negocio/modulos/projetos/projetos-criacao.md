...
| PROJ-CRIA-010 | A modal "Novo projeto" lista **possíveis projetos em 12 categorias** (Marca, Digital, Produto, Marketing, Conteúdo, IA & Automação, Negócios, Experiência, Comunicação, Operações, Audiovisual, Pesquisa & Inovação), num acordeão na lateral. O que a pessoa escolhe vira o **`nome`** do projeto — nunca o `tipo`. O `tipo` continua `startup`, único habilitado, e continua sendo ele quem decide o que o projeto abre por dentro. Sem escolha, o `nome` vem do catálogo, como antes. | Decidido 22/09/2026 com o Ricardo |
