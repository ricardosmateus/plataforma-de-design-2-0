...
(function () {
  ...
  /* ------------------------------------------------------------
     Por que um plano não é executável
     ------------------------------------------------------------
     São três causas distintas, e a tela dizia "a busca externa não
     está ligada" para todas — inclusive quando ela está ligada e
     funcionando. Errado de um jeito caro: manda a pessoa conferir uma
     configuração que já está certa, quando o que resolveria é mexer
     na descrição da tarefa.

     `conhecimento` é o caso mais comum e o menos óbvio: a pergunta é
     respondível por raciocínio, e PES-006 proíbe responder de memória
     sem fonte. Não é provedor faltando — é recusa deliberada, e dizer
     isso é o que permite à pessoa reformular. */
  function motivoNaoExecutavel(nivel) {
    if (nivel === 'navegacao') {
      return 'Ainda não entro em sites específicos como LinkedIn ou Instagram. ' +
             'Peça o fato em si e eu busco em fontes públicas.';
    }
    if (nivel === 'conhecimento') {
      return 'Esta descrição pede raciocínio, não um fato para verificar em fonte externa — ' +
             'e responder de memória, sem fonte, é justamente o que não fazemos. ' +
             'Reescreva a tarefa como uma pergunta verificável (com número, data, nome ou ' +
             'fonte) e a busca executa.';
    }
    if (nivel === 'lugares') {
      return 'Buscas com recorte geográfico ainda não estão ligadas nesta instalação. ' +
             'A mesma pergunta sem o recorte de bairro ou cidade eu consigo buscar.';
    }
    ...
    garantirSessao()
      ...
      .then(function (plano) {
        ...
        if (!plano.executavel) {
          ...
          desenharAlerta(motivoNaoExecutavel(plano.nivel));
          return null;
        }
    ...
    pesquisarNoQuadro: function (pergunta) {
      ...
      return garantirSessao()
        ...
        .then(function (plano) {
          ...
          if (!plano.executavel) {
            avisarNaTela(motivoNaoExecutavel(plano.nivel));
            return null;
          }
        ...
        .then(function (r) {
          ...
          /* A fonte inteira, nao so o rotulo: a associacao por assunto
             casa pela URL, e `f.titulo || f.url` joga a URL fora
             sempre que ha titulo — que e o caso comum. */
          ...
          /* Lugares continuam sendo cartões: cada um é um achado
             independente, com endereço e nota — é exatamente o que um
             post-it guarda bem. */
          ...
          console.log('[Pesquisa] quadros planejados:', planos.map(function (p) {
            return p.tipo + ' — "' + p.titulo + '"' +
              (p.tipo === 'documento'
                ? ' (' + p.texto.length + ' caracteres)'
                : ' (' + p.itens.length + ' cards)') +
              ' [' + ((p.fontes || []).length) + ' fontes]';
          }));
          ...
          /* Um quadro por assunto. O primeiro que voltar `null` diz que
             a tarefa esta concluida (BOARD-LEITURA-003) — nesse caso
             nenhum quadro entra, e dizer isso e melhor que sumir. */
          var criados = 0;
          ...
          /* Uma gravacao com o board inteiro na tela. Cada
             criarQuadro* ja pede autosave sozinho, mas aquele pedido
             sai no meio do lote: quando o primeiro quadro disparou, o
             ultimo ainda nao existia. O autosave em voo marca os
             seguintes como pendentes e regrava ao terminar, entao o
             estado final chega — este disparo e o que garante que ele
             chegue mesmo se o lote tiver um quadro so, ou se o
             primeiro pedido tiver saido antes de BoardAcoes ficar
             pronto. */
